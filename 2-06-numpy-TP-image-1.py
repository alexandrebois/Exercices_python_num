...
# %%
# votre code
im=np.ones(shape=(91,91,3), dtype=np.uint8)*255 #tableau blanc 
#plt.imshow(im)

im=np.zeros(shape=(91,91,3), dtype=np.uint8) #noir

im=np.ones(shape=(91,91,3), dtype=np.uint8)*255 
im[::, ::, 2]=0 #jaune

print(im[0,0], im[90,90]) #premier et dernier pixel
...
im2=im[a2-l//2:a2+l//2,b2-c//2:b2+c//2,::]
plt.imshow(im2);
...
im3=im[::,::,2]
plt.imshow(im1, cmap='Blues');

image=plt.imread('les-mines.jpg')
im=image.copy()

(a,b,c)=im.shape
im[a-200:,b-200:,0]=219
im[a-200:,b-200:,1]=112
im[a-200:,b-200:,2]=147
plt.imshow(im);
...
(a,b,c)=im.shape
im[a-200:,b-200:, ::]=255
im[a-200:,b-200::2,1]=0
im[a-200:,b-200::2,2]=0
im[a-200::2,b-200:,1]=0
im[a-200::2,b-200:,2]=0
...
image=plt.imread('les-mines.jpg')
(a,b,c)=image.shape
im=np.zeros(shape=(a,b,c+1), dtype=np.uint8)
im[::,::,:3]=image.copy()
im[::,::,3]=128
plt.imshow(im);
...
moy=np.add(np.add(im2[::,::,0],im2[:: , ::,1]),im2[::,::,2])/3
plt.imshow(moy, cmap='Greys');
plt.show()


#def y(l):
#    return 0.299*l[0]+0.587*l[1]+0.114*l[2]
#y([1,2,3])

#y_vect=np.vectorize(y)
#im3= y_vect(im2)  #renvoit une erreur 

imy=np.add(np.add(im2[::,::,0]*0.299,im2[:: , ::,1]*0.587),im2[::,::,2]*114)
plt.imshow(imy, cmap='Greys');
plt.show()
...
imrac=np.sqrt(moy)
plt.imshow(imrac, cmap='Greys');
plt.show()

imfinale=np.array(moy*255, dtype=np.uint8)
imfinale.dtype
#plt.imshow(imfinale, cmap='Greys')

# %% {"tags": ["raises-exception"]}
# %%timeit 
imf=plt.imread('les-mines.jpg')
...
axes[0,0].imshow(moy, cmap='Greys')
axes[0,1].imshow(imy, cmap='Greys');
axes[0,2].imshow(imrac, cmap='Greys');
axes[1,0].imshow(imrac, cmap='Greys');
axes[1,1].imshow(moy, cmap='Greys');
axes[1,2].imshow(imy, cmap='Greys');
axes[2,0].imshow(imy, cmap='Greys');
axes[2,1].imshow(imrac, cmap='Greys');
axes[2,2].imshow(moy, cmap='Greys');
fig.suptitle ( " en noir et blanc")
plt.tight_layout()
