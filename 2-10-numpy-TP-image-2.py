...
# %%
# 1)On stocke les données dans un dictionnaire 
...
#5)
# J'ai défini une "distance" sur les couleurs, une couleur à base de qqch est une couleur située à une distance de moins
# de 200 de la couleur cible
def distance(l1,l2):
    s=0
    for i in range(3):
        s+= abs(l1[i]-l2[i])
...
# du fait de l'ordre de calcul dans dot on a besoin de transposer la matrice de transition 
def sepia(im):
...
print(image.shape)
print(image.size)
# la taille est de 1279200 octets (=533*800*3)
...
#meme taille sur le disque 

#7)
plt.imshow(255-np.abs(np.add(plt.imread('imsave.jpg'),(plt.imread('save.jpg')*(-1))))); # je n'arrive pas à utiliser la fonction np.substract
# d'où la méthode alternative 
# De plus je n'ai pas de résultat visible si j'affiche seulement la différence (que du noir), j'affiche donc 255-|différence|
# pour avoir une image dominée par le blanc où l'on distingue des inhomogénéités
